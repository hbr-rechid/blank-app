...
if raw_uploaded_df is not None and not raw_uploaded_df.empty:
    ...
    default_name_col_idx = 0  # Padrão é a primeira coluna 

    # Tenta usar a coluna salva no Local Storage, se ela ainda for válida
    saved_name_col = st.session_state.get("pe_name_col_select")
    if saved_name_col and saved_name_col in cols:
        default_name_col_idx = cols.index(saved_name_col)
    else:
